...
func deleteRun(ctx context.Context, billingCredentialID int32) (err error) {
	// Create and authenticated client to the Taikun API
	myApiClient := tk.NewClient()

	// Execute a query into the API + graceful exit
	response, err := myApiClient.Client.OperationCredentialsAPI.OpscredentialsDelete(ctx, billingCredentialID).Execute()
	if err != nil {
