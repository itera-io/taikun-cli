...
func GetCloudTypeFromProjectID(projectID int32) (cloudType taikuncore.CloudType, err error) {
	myApiClient := tk.NewClient()
	data, response, err := myApiClient.Client.ProjectsAPI.ProjectsList(context.TODO()).Id(projectID).Execute()
	if len(data.GetData()) == 0 {
		return taikuncore.CLOUDTYPE_NONE, cmderr.ResourceNotFoundError("Project ID", projectID)
	}
	...
	switch data.GetData()[0].GetCloudType() {
	case taikuncore.CLOUDTYPE_AWS:
		return taikuncore.CLOUDTYPE_AWS, nil
	case taikuncore.CLOUDTYPE_AZURE:
		return taikuncore.CLOUDTYPE_AZURE, nil
	...
	case taikuncore.CLOUDTYPE_VSPHERE:
		return taikuncore.CLOUDTYPE_VSPHERE, nil
	default:
		return taikuncore.CLOUDTYPE_NONE, cmderr.ResourceNotFoundError("Cloud credential", projectID)
	}
