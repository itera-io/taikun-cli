...
func deleteRun(ctx context.Context, trustedRegistryID int32) (err error) {
	// Create and authenticated client to the Taikun API
	myApiClient := tk.NewClient()

	// Execute a query into the API + graceful exit
	response, err := myApiClient.Client.TrustedRegistriesAPI.TrustedregistriesDelete(ctx, trustedRegistryID).Execute()
	if err != nil {
