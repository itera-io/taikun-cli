...
var infoFields = fields.New(
	[]*field.Field{
		...
		field.NewVisible("DISPLAY-NAME", "displayName"),
		field.NewVisible("EMAIL", "email"),
		field.NewVisible("GLOBAL-ROLE", "globalRole"),
...
func NewCmdInfo() *cobra.Command {
	...
	cmd := cobra.Command{
		Use:   "info <ACCOUNT_ID> <USER_ID>",
		Short: "Get detailed information about a user in an account",
...
func infoRun(opts *InfoOptions) (err error) {
	...
	data, response, err := myApiClient.Client.AccountsAPI.AccountsAccountUserDetails(context.TODO(), opts.AccountID, opts.UserID).Execute()
	if err != nil {
