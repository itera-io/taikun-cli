Context 'project/vm/disk'
  setup() {
    ...
    id=$(taikun project add $(_rnd_name) -o $oid --cloud-credential-id $cc --flavors $flavor -I)
    img=$(taikun cc images $cc --no-decorate | grep -E -i '(ubuntu)|(focal)' | head -1 | cut -d ' ' -f 1)
    profile=$(taikun standalone-profile add $(_rnd_name) -o $oid --public-key "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHshx25CJGDd0HfOQqNt65n/970dsPt0y12lfKKO9fAs dummy" -I)
