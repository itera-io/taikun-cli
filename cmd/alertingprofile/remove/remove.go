...
func deleteRun(ctx context.Context, alertingProfileID int32) (err error) {
	// Create and authenticated client to the Taikun API
	myApiClient := tk.NewClient()

	// Execute a query into the API + graceful exit
	response, err := myApiClient.Client.AlertingProfilesAPI.AlertingprofilesDelete(ctx, alertingProfileID).Execute()
	if err != nil {
