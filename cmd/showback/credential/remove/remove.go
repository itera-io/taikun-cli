...
func deleteRun(ctx context.Context, showbackCredentialID int32) (err error) {
	// Create and authenticated client to the Taikun API
	myApiClient := tk.NewClient()

	// Execute a query into the API + graceful exit
	response, err := myApiClient.ShowbackClient.ShowbackCredentialsAPI.ShowbackcredentialsDelete(ctx, showbackCredentialID).Execute()
	if err != nil {
