...
func listRun(cmd *cobra.Command, opts *ListOptions) (err error) {
	...
	myRequest := myApiClient.Client.S3CredentialsAPI.S3credentialsList(ctx)
	if config.SortBy != "" {
	...
	for {
		...
		if int64(backupCredentialsCount) == data.GetTotalCount() { // casting is safe, extending maximum bounds
			break
		}
