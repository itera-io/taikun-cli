Context 'standaloneprofile/securitygroup'
  ...
  Example 'calling add without standalone profile ID should cause error'
    When call taikun standalone-profile security-group add --name "doom2" --protocol udp --min-port 666 --max-port 666 --remote-ip-prefix '0.0.0.0/0'
  ...
  Example 'calling add without max port should cause error'
    When call taikun standalone-profile security-group add "$id" --name "doom2" --protocol udp --min-port 666 --remote-ip-prefix '0.0.0.0/0'
    The status should equal 1
    The stderr should include 'max-port'
  End

  Example 'calling add without min port should cause error'
    When call taikun standalone-profile security-group add "$id" --name "doom2" --protocol udp --max-port 666 --remote-ip-prefix '0.0.0.0/0'
    The status should equal 1
    The stderr should include 'min-port'
  End

  Example 'adding ICMP with port range should cause error'
    When call taikun standalone-profile security-group add "$id" --name "doom2" --protocol icmp --min-port 666 --max-port 666 --remote-ip-prefix '0.0.0.0/0'
    The status should equal 1
    The stderr should include 'port range'
  End

  Example 'duplicate name should cause error'
    When call taikun standalone-profile security-group add "$id" --name "doom" --protocol udp --min-port 666 --max-port 666 --remote-ip-prefix '0.0.0.0/0'
    The status should equal 1
