...
func deleteRun(ctx context.Context, securityGroupID int32) (err error) {
	// Create and authenticated client to the Taikun API
	myApiClient := tk.NewClient()

	// Execute a query into the API + graceful exit
	response, err := myApiClient.Client.SecurityGroupAPI.SecuritygroupDelete(ctx, securityGroupID).Execute()
	if err != nil {
